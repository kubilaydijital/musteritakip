...
export default function Blog() {
  ...
  return <Layout><main className="page"><section className="container page-hero"><h1>İşinizi büyütecek içerikler</h1><p>Randevu yönetimi, müşteri takibi ve kaçan müşterileri geri kazanma üzerine pratik yazılar.</p></section><section className="container blog-grid">{blogPosts.map((post) => <article className="blog-card" key={post.slug}><div className="blog-icon">{post.image}</div><span>{post.date} • {post.read}</span><h3>{post.title}</h3><p>{post.excerpt}</p><Link to={`/blog/${post.slug}`}>Yazıyı oku →</Link></article>)}</section></main></Layout>
}
